...
2. **Agent auth (skippable)** — API-key fields for OpenAI / Anthropic / Google (Gemini) / OpenRouter, with the chosen agents' providers surfaced first; or a subscription-credential paste where the agent supports it (e.g. `claude setup-token`, Codex `~/.codex/auth.json`); or skip ("sign in in-shell later").
