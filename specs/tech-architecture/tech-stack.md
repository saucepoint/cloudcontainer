...
- Verification is green: 24 test files / 324 tests, all workspace type checks, lint, the browser build, the release dry-run, and `npm audit` pass.
...
- Selected-agent metadata and command-level read failures now fail daemon jobs instead of silently installing an incomplete credential set. Start/stop retries first inspect Incus desired state, so setup can resume after partial completion without repeating a completed transition.
