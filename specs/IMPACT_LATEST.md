...
2. Simplify the recent World ID 4.x browser/server flow while preserving account-bound proofs, upstream verification, and permanent nullifier uniqueness.
...
- World ID remains optional per deployment and remains interchangeable with administrator invites at the eligibility gate.
...
- Require the current `proofOfHuman` IDKit 4 preset with legacy fallback rather than the v3-only `orbLegacy` preset.
...
2. RED/GREEN: switch World ID to the current Proof of Human preset, pass deployment availability into the client, then simplify duplicated SDK types, telemetry, configuration parsing, and nullifier handling without weakening proof binding.
