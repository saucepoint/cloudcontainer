...
describe("drift correction (D1 <-> incus)", () => {
  ...
  it("preserves missing-container capacity across overlapping reconciliations", async () => {
    const { env } = makeEnv();
    await seedUser(env);
    await seedHost(env, { vcpu_allocated: 1, ram_allocated_mb: 3072, disk_allocated_gb: 20 });
    await seedContainer(env, { status: "running", cpu: 1, ram_mb: 1536, disk_gb: 5 });
    stubFetch(statsRoute([]));
    ...
    expect(host).toEqual({ vcpu_allocated: 1, ram_allocated_mb: 3072, disk_allocated_gb: 20 });
  });
  ...
  it("reuses the preserved reservation when retrying a missing container", async () => {
    ...
    expect(accounting).toEqual({ vcpu_allocated: 1, ram_allocated_mb: 3072, disk_allocated_gb: 20 });
  });
  ...
  it("releases a missing container's reservation once when destroy succeeds", async () => {
    ...
    expect(accounting).toEqual({ vcpu_allocated: 3, ram_allocated_mb: 1536, disk_allocated_gb: 10 });
  });
